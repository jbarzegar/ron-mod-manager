...
type Archive struct {
	...
	// ArchivePath holds the value of the "archivePath" field.
...
func (*Archive) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case archive.FieldName, archive.FieldArchivePath, archive.FieldMd5Sum:
...
func (a *Archive) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case archive.FieldArchivePath:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field archivePath", values[i])
			} else if value.Valid {
...
func (a *Archive) String() string {
	...
	builder.WriteString(a.Md5Sum)
	builder.WriteString(", ")
	builder.WriteString("installed=")
	builder.WriteString(fmt.Sprintf("%v", a.Installed))
	builder.WriteByte(')')
