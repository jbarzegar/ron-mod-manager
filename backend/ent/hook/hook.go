...
// The PakFunc type is an adapter to allow the use of ordinary
// function as Pak mutator.
type PakFunc func(context.Context, *ent.PakMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PakFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PakMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PakMutation", m)
}
