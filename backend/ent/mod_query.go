...
// QueryArchive chains the current query on the "archive" edge.
func (mq *ModQuery) QueryArchive() *ArchiveQuery {
	query := (&ArchiveClient{config: mq.config}).Query()
...
// QueryPaks chains the current query on the "paks" edge.
func (mq *ModQuery) QueryPaks() *PakQuery {
	query := (&PakClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *ModQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (mq *ModQuery) FirstIDX(ctx context.Context) int {
...
func (mq *ModQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (mq *ModQuery) OnlyIDX(ctx context.Context) int {
...
func (mq *ModQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (mq *ModQuery) IDsX(ctx context.Context) []int {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *ModQuery) Clone() *ModQuery {
	...
	return &ModQuery{
		...
		withArchive: mq.withArchive.Clone(),
		withPaks:    mq.withPaks.Clone(),
...
// WithArchive tells the query-builder to eager-load the nodes that are connected to
// the "archive" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *ModQuery) WithArchive(opts ...func(*ArchiveQuery)) *ModQuery {
	query := (&ArchiveClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withArchive = query
	return mq
}

// WithPaks tells the query-builder to eager-load the nodes that are connected to
// the "paks" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *ModQuery) WithPaks(opts ...func(*PakQuery)) *ModQuery {
	query := (&PakClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *ModQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Mod, error) {
	...
	if query := mq.withArchive; query != nil {
		if err := mq.loadArchive(ctx, query, nodes, nil,
			func(n *Mod, e *Archive) { n.Edges.Archive = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withPaks; query != nil {
		if err := mq.loadPaks(ctx, query, nodes,
			func(n *Mod) { n.Edges.Paks = []*Pak{} },
			func(n *Mod, e *Pak) { n.Edges.Paks = append(n.Edges.Paks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *ModQuery) loadArchive(ctx context.Context, query *ArchiveQuery, nodes []*Mod, init func(*Mod), assign func(*Mod, *Archive)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Mod)
	for i := range nodes {
		if nodes[i].mod_archive == nil {
			continue
		}
		fk := *nodes[i].mod_archive
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "mod_archive" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *ModQuery) loadPaks(ctx context.Context, query *PakQuery, nodes []*Mod, init func(*Mod), assign func(*Mod, *Pak)) error {
	...
	query.withFKs = true
	query.Where(predicate.Pak(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(mod.PaksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.mod_paks
		if fk == nil {
			return fmt.Errorf(`foreign-key "mod_paks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "mod_paks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mq *ModQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(mod.Table, mod.Columns, sqlgraph.NewFieldSpec(mod.FieldID, field.TypeInt))
	_spec.From = mq.sql
